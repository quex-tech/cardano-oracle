...
def main() -> None:
    ...
    args.func(context, wallet, validator, args)
...
@dataclass
class SingleOracleRepository:
    context: ChainContext
    validator: Validator

    def add_tx(self, oracle: Oracle, wallet: Wallet) -> Transaction:
...
@dataclass
class PrivateOracleRepository:
    context: ChainContext

    def add_tx(
        self, oracle: Oracle, pool_name: str, wallet: Wallet, oracles_pkh: VerificationKeyHash
    ) -> Transaction:
        policy = ScriptPubkey(wallet.vk.hash())
        ...
        builder.mint = pool.assets
        tx_out = TransactionOutput(
            Address(payment_part=oracles_pkh, network=nw),
            Value(2_000_000, pool.assets),
    ...
    def delete_tx(
        self, tx_input: TransactionInput, treasury_wallet: Wallet, oracles_wallet: Wallet
    ) -> Transaction | None:
        nw = self.context.network
        utxo = next(
            (u for u in self.context.utxos(oracles_wallet.addr(nw)) if u.input == tx_input),
            None,
        )
        ...
        builder = TransactionBuilder(self.context)
        builder.add_input_address(treasury_wallet.addr(nw))
        builder.add_input(utxo)
        builder.native_scripts = [ScriptPubkey(treasury_wallet.vk.hash())]
        builder.mint = MultiAsset() - utxo.output.amount.multi_asset

        return builder.build_and_sign(
            [treasury_wallet.sk, oracles_wallet.sk],
            change_address=treasury_wallet.addr(nw),
        )


def find_oracle_by_pk_pool_id(
    context: ChainContext,
    public_key: keys.PublicKey,
    pool_id: bytes,
...
def get_registered_oracles_at(
    ...
    for addr_payment_part in addr_payment_part_list:
        ...
        for utxo in context.utxos(addr):
            ...
            for pool in pools:
                yield RegisteredOracle(utxo.input, pool, oracle)
...
def list_oracles(
    ...
    for oracle in get_registered_oracles_at(
        ...
        pool = oracle.pool
        pool_type = (
            "single-oracle" if pool.currency_symbol == validator.currency_symbol else "private"
        )
        print("- UTxO:                 ", utxo)
        print(
            "  Pool:",
...
def add_oracle(
    context: ChainContext, wallet: OperatorWallet, validator: Validator, args: argparse.Namespace
) -> None:
    ...
    if args.pool_type == "single-oracle":
        single_oracle_repo = SingleOracleRepository(context, validator)
        signed_tx = single_oracle_repo.add_tx(oracle, wallet.treasury)
    else:
        private_oracle_repo = PrivateOracleRepository(context)
        signed_tx = private_oracle_repo.add_tx(
            oracle, args.pool_name, wallet.treasury, wallet.oracles.vk.hash()
        )
...
def delete_oracle(
    context: ChainContext, wallet: OperatorWallet, _: Validator, args: argparse.Namespace
) -> None:
    repo = PrivateOracleRepository(context)
    signed_tx = repo.delete_tx(args.utxo, wallet.treasury, wallet.oracles)
