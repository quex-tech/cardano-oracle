...
singleOraclePoolTypedValidator
  ...
  ctx@( ScriptContext
          ...
          (Redeemer rawRedeemer)
          _
        ) =
    let tokenName = TokenName . sha2_256 . serialiseData $ rawRedeemer
        (currencySymbol, ownAddress, maybeOldDatum) = validatorContext ctx
...
{-# INLINEABLE validatorContext #-}
validatorContext :: ScriptContext -> (CurrencySymbol, Address, Maybe Datum)
validatorContext
  (ScriptContext _ _ (MintingScript cs@(CurrencySymbol rawCS))) =
